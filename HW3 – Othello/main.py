...
def create_player(arg,  depht_or_time, turn):
    if arg == 'human':
        return agent.HumanPlayer()
    elif arg == 'random':
        return agent.RandomAgent()
    elif arg == 'minimax':
        return agent.MinimaxAgent(depht_or_time, turn)
    elif arg == 'alphabeta':
        return agent.AlphaBeta(depht_or_time, turn)
    elif arg == 'extra':
...
if __name__ == '__main__':
    ...
    player1 = create_player(get_arg(1), depht_or_time, 1)
    player2 = create_player(get_arg(2), depht_or_time, 2)
